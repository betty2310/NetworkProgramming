        ...
        while (true) {
            ...
            if (client_status == SELECT_IN_PROGRESS) {
                ...
                if (strcmp(recv_data, "")) {
                    ...
                    long file_size;
                    bytes_received = recv(conn_sock, &file_size, BUFF_SIZE, 0);
                    if (bytes_received <= 0) {
                        printf("Error in receiving data");
                        break;
                    } else {
                    ...
                    char *content = (char *) malloc(SEND_SIZE * sizeof(char));
                    int sum = 0;
                    while (1) {
                        bytes_received = recv(conn_sock, content, SEND_SIZE, 0);
                        if (bytes_received == 0) {
                            printf("Error: Gui File that bai \n");
                        }
                        sum += bytes_received;
                        fwrite(content, bytes_received, 1, fileptr);
                        free(content);
                        content = (char *) malloc(SEND_SIZE * sizeof(char));
                        if (sum >= file_size) {
                            break;
                        }
                    }
